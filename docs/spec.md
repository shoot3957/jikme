...
- 경기 날짜가 지나면 조회 시점에 자동으로 `CLOSED` 처리 — 단, `MATCHED` 상태인 글은 제외(작성자의 "직관 완료" 처리 대상이므로 매칭 상태를 유지하며, 완료 후에도 `MATCHED`로 남음)
...
**채팅 메시지 보관 정책 상세**: 유저가 채팅방을 "나가기" 하면 `RoomParticipant.leftAt`에 시각이 기록됨. 이후 이 유저가 `GET /api/rooms`(채팅방 목록)를 조회하면 `leftAt`이 `null`인 방만 나오므로 이 방은 목록에서 빠지고, `GET /api/rooms/:roomId/messages`는 `leftAt` 이전 메시지를 숨기고 그 이후 메시지만 반환한다. 메시지 자체는 DB에서 삭제되지 않고 상대방 화면에는 그대로 남아 있음.

**결정**: 같은 상대와 다시 DM을 시작하면(`POST /api/rooms`) 기존 `Room`을 재사용한다(새 Room을 만들지 않음). 이때 나갔던 쪽의 `RoomParticipant.leftAt`을 `null`로 되돌려 목록에 다시 노출시키고, 메시지 조회 시에도 전체 히스토리(나가기 이전 메시지 포함)가 다시 보이게 한다 — "나가기"는 목록에서 숨기는 소프트 나가기이고, 다시 대화를 시작하면 완전히 재입장하는 것으로 취급.
